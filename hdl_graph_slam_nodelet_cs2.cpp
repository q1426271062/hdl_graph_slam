...
public:
  ...
  virtual void onInit() {
    ...
    //by li 98-105
    pose_time_offset = private_nh.param<double>("pose_time_offset", 0.0);
    pose_edge_stddev_xy = private_nh.param<double>("pose_edge_stddev_xy", 10.0);
    pose_edge_stddev_z = private_nh.param<double>("pose_edge_stddev_z", 10.0);

    enable_pose_orientation = private_nh.param<bool>("enable_pose_orientation", false);
    pose_orientation_edge_stddev = private_nh.param<double>("pose_orientation_edge_stddev", 0.1);
    //
    ...
    if(private_nh.param<bool>("enable_pose", true)) {
      pose_sub = nh.subscribe("/pose_data/pose", 1024, &HdlGraphSlamNodelet::pose_callback, this);
      posepos_sub = nh.subscribe("/pose_data/posepos", 1024, &HdlGraphSlamNodelet::posepos_callback, this);
      poseori_sub = nh.subscribe("/pose_data/poseori", 1024, &HdlGraphSlamNodelet::poseori_callback, this);
      //poseori_sub = nh.subscribe("/pose_data/pose_ori", 1024, &HdlGraphSlamNodelet::poseori_callback, this);
    }
    //
...
private:
  ...
  //by li/////////////// 263-396
  /**
  void posepos_callback(const geometry_msgs::PointStampedConstPtr& posepos1_msg) {

      geometry_msgs::PoseStampedConstPtr posepos_msg(new geometry_msgs::PoseStamped());

      std::lock_guard<std::mutex> lock(posepos_queue_mutex);
      posepos_msg->header.stamp += ros::Duration(pose_time_offset);
      //posepos_msg->header = posepos1_msg->header.stamp;
      posepos_msg->pose.position.x = posepos1_msg->pose.position.x;//posepos_msg->position.z
      posepos_msg->pose.position.y = posepos1_msg->pose.position.y;
      posepos_msg->pose.position.z = posepos1_msg->pose.position.z;
      posepos_queue.push_back(posepos_msg);
    }
  void poseori_callback(const geometry_msgs::PoseStampedConstPtr& poseori1_msg) {
      geometry_msgs::PoseStampedConstPtr poseori_msg(new geometry_msgs::PoseStamped());
      //poseori_msg->header = poseori1_msg->header;
      std::lock_guard<std::mutex> lock(poseori_queue_mutex);
      poseori_msg->header = poseori1_msg->header.stamp;
      poseori_msg->pose.orientation.x = poseori1_msg->pose.orientation.x;//poseori_msg->orientation.w = poseori1_msg->pose.pose.orientation.w;
      poseori_msg->pose.orientation.y = poseori1_msg->pose.orientation.y;
      poseori_msg->pose.orientation.z = poseori1_msg->pose.orientation.z;
      poseori_msg->pose.orientation.w = poseori1_msg->pose.orientation.w;
      //pose_callback(pose_msg);
      poseori_queue.push_back(poseori_msg);
  }*/

  void pose_callback(const geometry_msgs::PoseStampedConstPtr& pose_msg){
  std::lock_guard<std::mutex> lock(pose_queue_mutex);
  pose_msg->header.stamp += ros::Duration(pose_time_offset);
  pose_queue.push_back(pose_msg);
  }
  /**void poseori_callback(const geometry_msgs::QuaternionStampedConstPtr& poseori_msg){
  std::lock_guard<std::mutex> lock(poseori_queue_mutex);
  poseori_msg->header.stamp += ros::Duration(pose_time_offset);
  poseori_queue.push_back(poseori_msg);
  }*/
      // //////
  bool flush_posepos_queue() {
    std::lock_guard<std::mutex> lock(pose_queue_mutex);

    /**
    if(keyframes.empty() || posepos_queue.empty()) {
      return false;
    }

    bool updated = false;
    auto posepos_cursor = posepos_queue.begin();

    for(auto& keyframe : keyframes) {
      if(keyframe->stamp > posepos_queue.back()->header.stamp) {
        break;
      }

      if(keyframe->stamp < (*posepos_cursor)->header.stamp ) {//|| keyframe->utm_coord || keyframe->acceleration) {
        continue;
      }

      // find the pose data which is closest to the keyframe
      auto closest_posepos = posepos_cursor;
      for(auto posepos = posepos_cursor; posepos != posepos_queue.end(); posepos++) {
        auto dt = ((*closest_posepos)->header.stamp - keyframe->stamp).toSec();
        auto dt2 = ((*posepos)->header.stamp - keyframe->stamp).toSec();
        if(std::abs(dt) < std::abs(dt2)) {
          break;
        }

        closest_posepos = posepos;
      }

      // if the time residual between the pose and keyframe is too large, skip it
      posepos_cursor = closest_posepos;
      if(0.2 < std::abs(((*closest_posepos)->header.stamp - keyframe->stamp).toSec())) {
        continue;
      }
      */

      //const auto& pose_pos = (*closest_pose)->pose.pose.position;
      //const auto& pose_ori = (*closest_pose)->pose.pose.orientation;
      /** // convert (latitude, longitude, altitude) -> (easting, northing, altitude) in UTM coordinate
      geodesy::UTMPoint utm;
      geodesy::fromMsg((*closest_gps)->position, utm);
      */
    // pose ///
    if(keyframes.empty() || pose_queue.empty()) {
      return false;
    }

    bool updated = false;
    auto pose_cursor = pose_queue.begin();

    for(auto& keyframe : keyframes) {
      if(keyframe->stamp > pose_queue.back()->header.stamp) {
        break;
      }

      if(keyframe->stamp < (*pose_cursor)->header.stamp ) {//|| keyframe->utm_coord || keyframe->acceleration) {
        continue;
      }

      // find the pose data which is closest to the keyframe
      auto closest_pose = pose_cursor;
      for(auto pose = pose_cursor; pose != pose_queue.end(); pose++) {
        auto dt = ((*closest_pose)->header.stamp - keyframe->stamp).toSec();
        auto dt2 = ((*pose)->header.stamp - keyframe->stamp).toSec();
        if(std::abs(dt) < std::abs(dt2)) {
          break;
        }
      ...
      // if the time residual between the pose and keyframe is too large, skip it
      pose_cursor = closest_pose;
      if(0.2 < std::abs(((*closest_pose)->header.stamp - keyframe->stamp).toSec())) {
        continue;
      }


      // pos ///
      //Eigen::Vector3d xyz(pose_msg->pose.position.x, (*closest_pose)->pose.position.y, (*closest_pose)->pose.position.z);
      //Eigen::Vector3d xyz(posepos_cursor.pose.position.x, posepos_cursor.pose.position.y, posepos_cursor.pose.position.z);
      Eigen::Vector3d xyz(pose_cursor.pose.position.x, pose_cursor.pose.position.y, pose_cursor.pose.position.z);

      // the first pose data position will be the origin of the map
      if(!zero_pos) {
        zero_pos = xyz;
      }
      xyz -= (*zero_pos); //keyframe和gps位姿直接相减得到先验约束加入到graph中

      keyframe->utm_coord = xyz;

      //ori//
      geometry_msgs::QuaternionStamped ori_pose; //quat_imu
      geometry_msgs::QuaternionStamped ori_base;

      ori_pose.header.frame_id = (*closest_pose)->header.frame_id;
      ori_pose.header.stamp = ros::Time(0);//ros::Time(0)表示使用缓冲中最新的tf数据
      //acc_imu.vector = (*closest_imu)->linear_acceleration;
      ori_pose.quaternion = (*closest_pose)->pose.orientation;

      try {
          tf_listener.transformQuaternion(base_frame_id, ori_pose, ori_base);
      ...
      if(std::isnan(xyz.z())) { //判断gps z轴是否有值，没有就只进行xy约束
        Eigen::Matrix2d information_matrix = Eigen::Matrix2d::Identity() / pose_edge_stddev_xy;
        edge = graph_slam->add_se3_prior_xy_edge(keyframe->node, xyz.head<2>(), information_matrix);
      } else {
        Eigen::Matrix3d information_matrix = Eigen::Matrix3d::Identity();
        information_matrix.block<2, 2>(0, 0) /= pose_edge_stddev_xy;
        information_matrix(2, 2) /= pose_edge_stddev_z;
        edge = graph_slam->add_se3_prior_xyz_edge(keyframe->node, xyz, information_matrix);
      }
      graph_slam->add_robust_kernel(edge, private_nh.param<std::string>("pose_edge_robust_kernel", "NONE"), private_nh.param<double>("pose_edge_robust_kernel_size", 1.0));

      // g2o ori ///
      keyframe->orientation = Eigen::Quaterniond(ori_base.quaternion.w, ori_base.quaternion.x, ori_base.quaternion.y, ori_base.quaternion.z);
      ...
      if(enable_pose_orientation) { //if(x) x=true or false
        Eigen::MatrixXd info = Eigen::MatrixXd::Identity(3, 3) / pose_orientation_edge_stddev;
        auto edge = graph_slam->add_se3_prior_quat_edge(keyframe->node, *keyframe->orientation, info);
        graph_slam->add_robust_kernel(edge, private_nh.param<std::string>("pose_orientation_edge_robust_kernel", "NONE"), private_nh.param<double>("pose_orientation_edge_robust_kernel_size", 1.0));
      }

      updated = true;
    }

    //auto remove_loc = std::upper_bound(posepos_queue.begin(), posepos_queue.end(), keyframes.back()->stamp, [=](const ros::Time& stamp, const geometry_msgs::PoseStampedConstPtr& posepos) { return stamp < posepos->header.stamp; });
    //posepos_queue.erase(posepos_queue.begin(), remove_loc);
    auto remove_loc = std::upper_bound(pose_queue.begin(), pose_queue.end(), keyframes.back()->stamp, [=](const ros::Time& stamp, const geometry_msgs::PoseStampedConstPtr& posepos) { return stamp < posepos->header.stamp; });
    pose_queue.erase(pose_queue.begin(), remove_loc);
    return updated;
  }

  // ori ///
  /**
  bool flush_poseori_queue() {
    std::lock_guard<std::mutex> lock(poseori_queue_mutex);

    if(keyframes.empty() || poseori_queue.empty()) {
      return false;
    }

    bool updated = false;
    auto poseori_cursor = poseori_queue.begin();

    for(auto& keyframe : keyframes) {
      if(keyframe->stamp > poseori_queue.back()->header.stamp) {
        break;
      }

      if(keyframe->stamp < (*poseori_cursor)->header.stamp ) {//|| keyframe->utm_coord || keyframe->acceleration) {
        continue;
      }

      // find the pose data which is closest to the keyframe
      auto closest_poseori = poseori_cursor;
      for(auto poseori = poseori_cursor; poseori != poseori_queue.end(); poseori++) {
        auto dt = ((*closest_poseori)->header.stamp - keyframe->stamp).toSec();
        auto dt2 = ((*poseori)->header.stamp - keyframe->stamp).toSec();
        if(std::abs(dt) < std::abs(dt2)) {
          break;
        }

        closest_poseori = poseori;
      }

      // if the time residual between the pose and keyframe is too large, skip it
      poseori_cursor = closest_poseori;
      if(0.2 < std::abs(((*closest_poseori)->header.stamp - keyframe->stamp).toSec())) {
        continue;
      }
     //geometry_msgs::Vector3Stamped pos_pose;
     //geometry_msgs::Vector3Stamped pos_base;
     geometry_msgs::QuaternionStamped ori_pose; //quat_imu
     geometry_msgs::QuaternionStamped ori_base;

     ori_pose.header.frame_id = (*closest_poseori)->header.frame_id;
     ori_pose.header.stamp = ros::Time(0);//ros::Time(0)表示使用缓冲中最新的tf数据
     //acc_imu.vector = (*closest_imu)->linear_acceleration;
     ori_pose.quaternion = (*closest_poseori)->pose.orientation;
  ...
  auto remove_loc = std::upper_bound(poseori_queue.begin(), poseori_queue.end(), keyframes.back()->stamp, [=](const ros::Time& stamp, const geometry_msgs::PoseStampedConstPtr& poseori) { return stamp < poseori->header.stamp; });
  poseori_queue.erase(poseori_queue.begin(), remove_loc);
...
private:
  ...
  ros::Subscriber posepos_sub;
  ros::Subscriber poseori_sub;
  //
  ...
  double pose_edge_stddev_xy;
  double pose_edge_stddev_z;
  bool enable_pose_orientation;
  double pose_orientation_edge_stddev;
  boost::optional<Eigen::Vector3d> zero_pos;
  std::mutex pose_queue_mutex;
  std::deque<geometry_msgs::PoseStampedConstPtr> pose_queue;
  std::mutex posepos_queue_mutex;
  std::deque<geometry_msgs::PoseStampedConstPtr> posepos_queue;
  std::mutex poseori_queue_mutex;
  std::deque<geometry_msgs::PoseStampedConstPtr> poseori_queue;
