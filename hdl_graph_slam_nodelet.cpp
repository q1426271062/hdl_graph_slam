...
// by li
    ...
    enable_pose = private_nh.param<bool>("enable_pose", false);
    enable_pose_orientation = private_nh.param<bool>("enable_pose_orientation", true);
...
// by li
    if(private_nh.param<bool>("enable_pose", true)) {
      pose_sub = mt_nh.subscribe("/pose_data/pose", 1024, &HdlGraphSlamNodelet::pose_callback, this);
    }
...
//by li
  void pose_callback(const geometry_msgs::PoseStampedConstPtr& pose_msg){
    ...
    //pose_msg->header.stamp += ros::Duration(0.0);
  ...
  bool flush_posepos_queue() {
    ...
    for(auto& keyframe : keyframes) {
      ...
      geometry_msgs::PointStamped pos_pose;
      pos_pose.header.frame_id = (*closest_pose)->header.frame_id;
      pos_pose.header.stamp = ros::Time(0);
      pos_pose.point = (*closest_pose)->pose.position;
      Eigen::Vector3d xyz(pos_pose.point.x, pos_pose.point.y, pos_pose.point.z);
      if(!zero_pos) {
        zero_pos = xyz;
      }
      xyz -= (*zero_pos);
